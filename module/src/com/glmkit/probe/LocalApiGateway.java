...
public class LocalApiGateway {
    ...
    private static String getWebUI() {
        ...
        return "<!DOCTYPE html><html lang=\"zh\"><head><meta charset=\"utf-8\">"
            ...
            + "<button onclick=\"copyLogs()\" style=\"margin-left:8px\">📋 一键复制</button>"
            + "</div>"
            ...
            + "fetch(B+'/v1/logs').then(r=>r.json()).then(d=>{"
            + "let text=d.logs||'(空)';"
            + "navigator.clipboard.writeText(text).then(()=>{alert('日志已复制到剪贴板');}).catch(()=>{"
            + "let ta=document.createElement('textarea');ta.value=text;document.body.appendChild(ta);ta.select();"
            + "try{document.execCommand('copy');alert('日志已复制到剪贴板');}catch(e){alert('复制失败，请手动选择');}"
